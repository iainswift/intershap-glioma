...
def custom_collate_fn(batch):
    ...
    wsi_file_names = [item['wsi_file_name'] for item in batch]
...
def evaluate(model, val_dataloader, criterion, summary_writer, device, epoch, num_classes, task='survival_prediction', mode='val', target_label='label'):
    ...
    for b_idx, batch_dict in enumerate(val_dataloader):
        ...
        patch_bags = batch_dict['patch_bag']
        ...
        with torch.no_grad():
            ...
            # Process variable-sized bags
            batch_outputs = []
            for i in range(len(patch_bags)):
                patches = patch_bags[i].to(device)
                # Add batch dimension: (bag_size, c, h, w) -> (1, bag_size, c, h, w)
                patches = patches.unsqueeze(0)
                rna = rna_data[i:i+1]
                output = model(patches, rna)
...
def train_model(model, dataloaders, criterion, optimizer, device, save_dir='checkpoints/models/',
    ...
    for epoch in range(num_epochs):
        ...
        for b_idx, batch_dict in enumerate(dataloaders['train']):
            ...
            # statistics
            # Calculate total patches processed
            current_batch_size = sum(pb.size(0) for pb in patch_bags)
            running_loss += loss.item() * current_batch_size

            inputs_seen += current_batch_size
            ...
            if (summary_step % log_interval == 0):
                ...
                speed_to_log = log_interval * current_batch_size / (time.time() - last_time)
