...
###############################################################################
### Example command
### $ python 2_GeneExpress_savescore.py --config "/path/to/config_rna_savescore.json"
###################################################
###################################################
...
def evaluate(model, val_dataloader, device, num_classes, mode='val', task='survival_prediction'):
...
def main():
    ...
    dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=config ['batch_size'], sampler=image_samplers[x],num_workers=1)
        for x in
    ...
    for dataset in list(image_datasets.keys()):
        ...
        dataset_loss, case_CI, output = evaluate(model,dataloaders_dict[dataset], device, config['num_classes'], mode='val', task=config['task'])
        ...
        outname = config['flag']
        if 'cv' in outname:
            output.to_csv(config['output_path']+config['model_path'].split("/")[-1]+"_rna_"+dataset+"_"+outname+"_df.csv")
        else:
            output.to_csv(config['output_path']+config['model_path'].split("/")[-1]+"_rna_"+dataset+"_df.csv")
