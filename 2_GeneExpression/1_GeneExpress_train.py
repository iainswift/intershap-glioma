...
def evaluate(model, val_dataloader, device, epoch, num_classes, mode='val', task='survival_prediction'):
    ...
    for b_idx, batch_dict in enumerate(val_dataloader):
        ...
        inputs = batch_dict['rna_data'].to(device, non_blocking=True)
...
def train_model(model, dataloaders, optimizer, device, num_epochs=25, num_classes=4,
                ...
                save_dir='checkpoints/models', output_dir=None, use_amp=None):
    ...
    for epoch in range(num_epochs):
        ...
        for b_idx, batch in enumerate(dataloaders['train']):
            ...
            # ACCELERATION: Added non_blocking=True
            inputs = batch['rna_data'].to(device, non_blocking=True)
            survival_months = batch['survival_months'].to(device, non_blocking=True).float()
            survival_bin = batch['survival_bin'].to(device, non_blocking=True).long()
            vital_status = batch['vital_status'].to(device, non_blocking=True).float()
