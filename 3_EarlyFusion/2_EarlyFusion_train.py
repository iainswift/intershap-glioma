...
###############################################################################
### Example command
### $ CUDA_VISIBLE_DEVICES=2,3 python 2_EarlyFusion_train.py --config "/path/to/config_early_train.json"
###################################################
###################################################
...
def train_model(model, dataloaders, optimizer, device, num_epochs=25, num_classes=4,
                ...
                save_dir='checkpoints/models', save=True):
...
def main():
    ...
    #train_loss, _ = evaluate(model, dataloaders_dict['train'], device, '-1', num_classes, task=task, mode='train')
    #val_loss, _ = evaluate(model, dataloaders_dict['val'], device, '-1', num_classes, task=task, mode='val')
