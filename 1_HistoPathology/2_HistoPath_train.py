...
def evaluate(model, val_dataloader, criterion, summary_writer, device, epoch, num_classes, mode='val', task='survival_prediction', target_label='label'):
    ...
    for b_idx, batch_dict in enumerate(val_dataloader):
        ...
        # ACCELERATION: Added non_blocking=True
        inputs = batch_dict['patch_bag'].to(device, non_blocking=True)
        labels = batch_dict[target_label].to(device, non_blocking=True).long()

        # for binary classification, target_label = 'label', for multiclass target_label = 'label_multiclass'
        wsis = batch_dict['WSI']
        survival_months = batch_dict['survival_months'].to(device, non_blocking=True).float()
        survival_bin = batch_dict['survival_bin'].to(device, non_blocking=True).long()
        vital_status = batch_dict['vital_status'].to(device, non_blocking=True).float()
...
def train_model(model, dataloaders, criterion, optimizer, device, save_dir='checkpoints/models/',
                ...
                use_amp=None): # ACCELERATION: Added use_amp argument
    
    best_val_loss = np.inf
    summary_step = 0
    best_epoch = -1

    # ACCELERATION: Mixed precision training setup
    if use_amp is None:
        use_amp = device.type == 'cuda'

    # Use new amp API when available to avoid deprecation warnings
    try:
    ...
    if use_amp:
        print("Mixed precision (AMP) enabled for faster training on GPU")
...
def main():
    ...
    device = torch.device("cuda:0" if (torch.cuda.is_available() and config.get('use_cuda', True)) else "cpu")
    num_classes, num_epochs = config['num_classes'], config['num_epochs']

    # ACCELERATION: GPU optimizations
    if torch.cuda.is_available() and config.get('use_cuda', False):
        try:
            # For deterministic runs prefer disabling benchmark and enabling deterministic
            # Check if a 'deterministic' flag exists in config, otherwise default to True for safety
    ...
    # ACCELERATION: Dataset optimization flags
    pin_memory = True if (device.type == 'cuda' and config.get('use_cuda', False)) else False
    persistent_workers = True if config.get('num_workers', 0) > 0 else False

    # Create training and validation dataloaders
    # ACCELERATION: Updated kwargs for pin_memory and persistent_workers
    dataloaders_dict = {
    ...
    # ACCELERATION: Determine AMP override
    use_amp_flag = None if getattr(args, 'use_amp', -1) == -1 else bool(getattr(args, 'use_amp',  -1))
    ...
    train_model(model=model, dataloaders=dataloaders_dict, criterion=criterion,
                ...
                use_amp=use_amp_flag # ACCELERATION: Passed flag
               )
    end=time.time()
...
# ACCELERATION: Added flag
parser.add_argument("--use-amp", type=int, default=-1, help='Force AMP: 1 enable, 0 disable, -1 auto')
