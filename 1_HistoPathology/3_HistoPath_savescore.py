...
###############################################################################
### Example command
### $ python 3_HistoPath_savescore.py --config "/path/to/config_ffpe_savescore.json"
###################################################
###################################################
...
def evaluate(model, val_dataloader, device, criterion, num_classes, mode='val', task='survival_prediction'):
    ...
    for b_idx, batch_dict in enumerate(val_dataloader):
        ...
        print(f"Processing batch {b_idx}")
        ...
        # forward
        with torch.no_grad():
            # OPTIMIZATION: Use Automatic Mixed Precision (AMP)
...
def main():
    ...
    # OPTIMIZATION: Added pin_memory=True
    dataloaders_dict = {
    ...
    print("Testing dataloader - attempting to load first batch...")
